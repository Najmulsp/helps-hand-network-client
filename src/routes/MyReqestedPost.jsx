...
const MyReqestedPost = () => {
  ...
  useEffect(() => {
    if (user.email) {
      axiosSecure.get(`/MyRequestPost?email=${user?.email}`).then(data => {
        console.log(data.data);
        setMyRequestPosts(data.data);
      });
    }
  }, [user, axiosSecure]);
...
/deleteMyRequestPost/${id}`,
          ...
          .then(data => {
            ...
            if (data.deletedCount > 0) {
              const remaining = myRequestPosts.filter(
  ...
  return (
    <div className="container p-2 mx-auto sm:p-4 dark:text-gray-800">
      ...
      <h2 className="mb-4 text-2xl font-semibold leading-tight">
        <span className="font-bold">These are Your Requsted Post</span> <br />{' '}
        You can also cancel your post here
      </h2>
          ...
          {myRequestPosts?.map(myRequestPost => (
            <tbody key={myRequestPosts?._id}>
              <tr className="border-b border-opacity-20 dark:border-gray-300 dark:bg-gray-50">
                ...
                <td className="p-3">
                  <p>
                    {new Date(myRequestPost?.deadline).toLocaleDateString()}
                  </p>
                </td>
                <td className="p-3">
                ...
                <td className="p-3 pl-0 text-left">
                  <button
                    onClick={() => handleCancelPost(myRequestPost?._id)}
                    className="btn bg-gradient-to-r from-orange-600 via-orange-500 to-orange-700 hover:bg-gradient-to-br focus:ring-purple-300"
                  >
                    Cancel
                  </button>
                </td>
